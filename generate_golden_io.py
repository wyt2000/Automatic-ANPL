...
async def request(semaphone, client, io_request_kwargs, *args):
    async with semaphone:
        io = await client.request_for_golden_io(**io_request_kwargs)
        return io 

if __name__ == '__main__':
    argparser = argparse.ArgumentParser()
    argparser.add_argument("-p", "--num_problems", help="Number of problems", type=int, default=1)
    argparser.add_argument("-s", "--num_samples", help="Number of golden io for each problem", type=int, default=1)
    ...
    save_dir = f'anpl_apps_golden_io_{timestr}/'
