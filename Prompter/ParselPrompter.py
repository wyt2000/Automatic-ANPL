...
{solution}
# You should return Parsel code consist with Parsel grammar mentioned above, whose functions are defined by natural language in only one single line for each function! The logic of each function should be as simple as possible, if the logic is too complex, you should split it into subfunctions without changing or missing the semantics. There should be only one top function in Parsel code, which has no indentation. You shouldn't output any Python code! You shouldn't copy the examples above!"""
